...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="NiceMeta - Open-Source Business Intelligence Platform",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  nicemeta                      Run the application
  nicemeta db upgrade           Apply all pending migrations
  nicemeta db upgrade head      Apply all pending migrations
  nicemeta db downgrade -1      Rollback one migration
  nicemeta db revision -m "Add new table"  Create a new migration
  nicemeta db current           Show current database version
  nicemeta db history           Show migration history
        """
    )
