...
def generate_template(spec: dict[str, Any]) -> str:
    ...
    extract_strategy = spec.get("extract_strategy") or (
        "incremental" if load_mode == "incremental"
        else "streaming" if load_mode == "streaming"
        else "full"
    )
    write_behavior = spec.get("write_behavior") or LOAD_MODE_TO_DISPOSITION.get(load_mode, "replace")
    disposition = write_behavior if write_behavior in ("replace", "append", "merge") else "replace"
...
def _disposition_kwargs(
    load_mode: str,
    primary_key: str,
    cursor_field: str,
    write_behavior: str | None = None,
) -> str:
    """Build the kwargs for ``@dlt.resource`` based on write behavior.

    Extraction strategy (incremental cursor) is applied separately so
    incremental extract + merge write can be combined.
    """
    parts: list[str] = []
    disposition = write_behavior or LOAD_MODE_TO_DISPOSITION.get(load_mode, "replace")
    if disposition not in ("replace", "append", "merge"):
        disposition = "replace"
    parts.append(f'write_disposition="{disposition}"')
    if disposition == "merge" and primary_key:
...
Load mode: {t["load_mode"]}   ·   extract: {t.get("extract_strategy", "full")}   ·   write: {t.get("write_behavior") or t["load_mode"]}
Destination: {t["dest"].get("name", "?")} ({dest_type})

Generated code. Convert to custom in the Definition tab to freeze edits;
configuration changes will not overwrite custom Python.
"""
...
def _template_rest_api(t: dict[str, Any]) -> str:
    ...
    res_decorator = (
        "@dlt.resource("
        + _disposition_kwargs(
            t["load_mode"], t["primary_key"], cursor_field, t.get("write_behavior"),
        )
        + ")"
    )
    newline = "\n"
    ...
    auth_line = (
        "headers['Authorization'] = (ctx.source_config or {}).get('auth_header') or ''"
    )
...
@dlt.resource({_disposition_kwargs(t["load_mode"], t["primary_key"], t["cursor_field"], t.get("write_behavior"))})
def rows() -> Iterator[dict]:
    url = (ctx.source_config or {{}}).get("connection_url") or "sqlite:///source.db"
